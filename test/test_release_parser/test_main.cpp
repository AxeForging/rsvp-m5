...
const char *kAsset = "rsvp-m5-core2-ota.bin";
...
void test_extracts_tag_and_matching_asset_url() {
  const String json =
      ...
      "{\"name\":\"rsvp-m5-core2-ota.bin\",\"browser_download_url\":\"https://example.com/ota.bin\"}"
      "]}";
  releaseparser::ReleaseInfo out;
...
void test_picks_url_after_matching_name_not_a_neighbor() {
  ...
  const String json =
      ...
      "{\"name\":\"rsvp-m5-core2-ota.bin\",\"browser_download_url\":\"https://example.com/correct.bin\"}"
      "]}";
  const releaseparser::ReleaseInfo out = parseJson(json);
