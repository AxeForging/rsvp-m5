...
constexpr const char *kMdnsName = "rsvp-m5";
...
bool CompanionSyncManager::startAccessPoint() {
  const String ssid = "RSVP-M5-" + deviceSuffix();
...
bool CompanionSyncManager::startServer() {
  ...
  // Captive portal: in AP mode resolve every DNS query to our IP so phones auto-open the
  // landing page after joining. mDNS advertises rsvp-m5.local on both AP and station.
  if (networkMode_ == NetworkMode::AccessPoint) {
    dnsServer_.setErrorReplyCode(DNSReplyCode::NoError);
    dnsStarted_ = dnsServer_.start(53, "*", WiFi.softAPIP());
  }
...
void CompanionSyncManager::handleInfo() {
  ...
  const String body = String("{") + "\"name\":\"RSVP M5\"," +
                      "\"mode\":\"" + mode + "\"," +
...
bool CompanionSyncManager::writeRssFeedsJson(const String &body, String &error) {
  ...
  file.println("# RSVP M5 RSS feeds");
